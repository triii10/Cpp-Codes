...
string parsedString(string S)
{
    if(S.empty())
        return "";
    /**
     * @param no --> stores the number of times the string insider the bracket will be repeated
     * @param last --> stores the index of the last character in the string
    */
    int no, last = S.length()-1;        
    /**
     * @param number --> stores "@no" in string format to be converted later and stored in "@no"
     * @param toParse --> stores the string inside the bracket that is to be recursively parsed
     * @param parsed --> the string is parsed recursively and stored in this variable
    */
